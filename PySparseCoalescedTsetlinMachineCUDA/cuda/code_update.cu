...
extern "C"
{
    ...
    number_of_include_actions (unsigned int *ta_state)
    {
    ...
    inc (unsigned int *ta_state, int clause, int chunk, unsigned int active)
    {
    ...
    // active bit vector.
    __device__ inline void
    dec (unsigned int *ta_state, int clause, int chunk, unsigned int active)
    {
        unsigned int carry, carry_next;
        int id = clause * LA_CHUNKS * STATE_BITS + chunk * STATE_BITS;
        carry = active;
